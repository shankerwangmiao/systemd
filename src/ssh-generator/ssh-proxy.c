...
static int process_vsock_mux(const char *path, const char *port) {
        ...
        int old_peek_off;
        r = getsockopt_int(fd, SOL_SOCKET, SO_PEEK_OFF, &old_peek_off);
        if (r < 0)
                return log_error_errno(-r, "Failed to get socket SO_PEEK_OFF for %s: %m", path);
        r = setsockopt_int(fd, SOL_SOCKET, SO_PEEK_OFF, 0);
        if (r < 0)
                return log_error_errno(-r, "Failed to set socket SO_PEEK_OFF for %s: %m", path);
        ...
        for (;;) {
                if (pos >= bytes_avail) {
                        if (bytes_avail < sizeof(recv_buf)) {
                                ...
                                if (len < 0) {
                                        if (errno == EAGAIN) {
                                                log_info("Timeout while waiting for OK PORT response from %s", path);
                                                log_info("Assume the multiplexer will not send OK PORT.");
                                                goto passout_fd;
                                        } else
                                        ...
                                        if (pos == 0) {
                                                ...
                                                log_debug("Partial data received, but connection closed before full OK PORT response.");
                                                log_debug("Assume the response is not valid OK PORT.");
                                                log_debug("Give out fd without further processing.");
                                                goto passout_fd;
                                        }
                                }
                                bytes_avail += len;
                ...
                if (pos < expected_prefix_len && recv_buf[pos] != expected_prefix[pos]) {
                        log_debug("Received response does not match expected OK PORT response from %s", path);
                        log_debug("Give out fd without further processing.");
                        goto passout_fd;
...

        {
                ...
                ssize_t len = recv(fd, buf_discard, pos, 0);
                if (len < 0)
...
passout_fd:
        {
                r = setsockopt_int(fd, SOL_SOCKET, SO_PEEK_OFF, old_peek_off);
                if (r < 0)
                        return log_error_errno(-r, "Failed to restore socket SO_PEEK_OFF for %s: %m", path);
        }
        {
